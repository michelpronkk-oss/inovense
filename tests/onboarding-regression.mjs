...
assert.match(page, /padStart\(2, "0"\).*\/ 06/, "onboarding must expose six-step progress");
...
assert.match(page, /Microsoft 365/, "systems must include Microsoft 365");
assert.match(page, /Zendesk/, "systems must include Zendesk");
assert.match(page, /Trello/, "systems must include Trello");
assert.match(page, /Support Operator/, "readiness must include Support Operator");
assert.match(page, /api\/operators\/\$\{draft\.priority\}\/activate/, "monitoring must use the existing activation route");
assert.match(page, /api\/connectors\/accounts/, "connection status must be based on real connector truth");
...
assert.match(actions, /"support"/, "support must be a valid first capability");
