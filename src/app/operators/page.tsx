...
export const metadata: Metadata = {
  ...
  description: "Meet the current Auterim operators for revenue, client flow and operations. Each works from approved business context and follows clear approval boundaries.",
  alternates: { canonical: "https://auterim.com/operators" },
  openGraph: { url: "https://auterim.com/operators", title: "AI Operator Registry | Auterim", description: "Meet the current Auterim operators for revenue, client flow and operations, built around approved business context and clear control boundaries.", type: "website", images: [{ url: "/og/og-operators.png", width: 1200, height: 630, alt: "Auterim AI Operator Registry" }] },
  twitter: { card: "summary_large_image", title: "AI Operator Registry | Auterim", description: "Meet the current Auterim operators for revenue, client flow and operations, built around approved business context and clear control boundaries.", images: [{ url: "/og/og-operators.png", width: 1200, height: 630, alt: "Auterim AI Operator Registry" }] },
};
...
export default function OperatorsPage() {
  return <div className="auterim-v3-page operators-registry-page"><V3Header /><main>
    <section className="page-hero operators-registry-hero"><div className="wrap"><div className="close-main"><span className="lbl"><i aria-hidden="true" />The AI workforce</span><h1>Defined roles for business work that cannot be left to a chat.</h1><p>An operator monitors approved systems, understands the job it owns, prepares the next action, pauses when judgment is needed, and records what happened.</p><div className="close-actions"><Link href="/getting-started" className="btn btn-a">See how deployment starts <span className="arrow">→</span></Link><Link href="/agents" className="btn btn-b">Why operators differ from AI agents</Link></div></div></div></section>

    <section className="sec operators-explainer"><div className="wrap"><div className="operators-explainer-copy"><span className="lbl">How an operator works</span><h2>A role, a loop, and a visible boundary.</h2><p className="say">Auterim does not give a generic assistant broad access and hope for the best. Each operator has a business outcome, a defined set of systems, and a policy decision before any consequential action proceeds.</p></div><ol className="operators-loop" aria-label="Operator operating loop">{["Detect", "Prepare", "Approve", "Execute", "Log"].map((step, index) => <li key={step}><span>{String(index + 1).padStart(2, "0")}</span><strong>{step}</strong><p>{loopDescriptions[index]}</p></li>)}</ol></div></section>
