...
import { OnboardingJourney } from "@/components/home-v3/onboarding-journey";
import { EarlyAccessFacts } from "@/components/home-v3/onboarding-facts";
...
const facts = [
  { label: "No account, no trial", body: "Requesting Early Access does not create an account, start a trial, or begin billing." },
  { label: "Invitation before setup", body: "An invited workspace connects systems and sets policy before anything runs." },
  { label: "Trial starts explicitly", body: "The three-day trial only begins when your workspace chooses to start it." },
  { label: "Reviewed before scaling", body: "You check the first prepared work before deciding what happens next." },
] as const;

const journeyGroups = [
  {
    label: "An invitation to begin.",
    description: "Tell us about the work. If the use case fits, we will help you take the next step.",
    steps: [
  ...
  {
    label: "A workspace under your rules.",
    description: "Connect the right systems, activate a role, and set its policies.",
    steps: [
  ...
  {
    label: "Your first prepared work.",
    description: "Review the evidence and the next move. Start a trial only when you choose.",
    steps: [
...
export default function GettingStartedPage() {
  return <PublicSiteFrame>
    ...
    <section className="sec public-content getting-started-facts-section">
      <div className="wrap">
        <span className="lbl">Early access</span>
        <p className="sec-lead">Four things are true before you connect anything.</p>
        <div className="rv"><EarlyAccessFacts items={facts} /></div>
      </div>
    </section>

    <StorySection className="getting-started-path" label="From request to reviewed work" title="Start small. Set the boundary. See the work.">
      <div className="rv"><OnboardingJourney groups={journeyGroups} /></div>
    ...
    <div className="rv"><PublicCta title="Bring one workflow into focus." description="Tell us what your team wants to move first." /></div>
    <RevealMount />
  </PublicSiteFrame>;
}
