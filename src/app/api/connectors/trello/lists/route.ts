...
import { listTrelloLists, TrelloExecutionError } from "@/lib/operators/executors/trello";
...
export async function GET(req: NextRequest) {
  ...
  try {
    return NextResponse.json({ lists: await listTrelloLists(context.workspaceId, boardId) });
  } catch (error) {
    if (error instanceof TrelloExecutionError) {
      return NextResponse.json({ error: error.details.code || "trello_lists_failed", message: error.message, details: error.details }, { status: error.details.status ?? 502 });
    }
    return NextResponse.json({ error: "trello_lists_failed", message: error instanceof Error ? error.message : "Could not load Trello lists." }, { status: 502 });
  }
}
