...
import { buildHubSpotAuthorizationUrl, getHubSpotConfigStatus } from "@/lib/connectors/hubspot";
...
export async function GET(req: NextRequest) {
  ...
  if (!getHubSpotConfigStatus().configured) return NextResponse.json({ error: "HubSpot is not configured yet. Contact support.", code: "not_configured" }, { status: 503 });
  ...
  if (!context.userEmail) return NextResponse.json({ error: "A verified account email is required to connect HubSpot." }, { status: 400 });
  ...
  if (!workspace.data.can_use_real_connectors || workspace.data.billing_status === "preview") return NextResponse.redirect(new URL("/pricing?gate=connectors&source=hubspot", getAppUrl()));
  return NextResponse.redirect(buildHubSpotAuthorizationUrl(createProviderOAuthState("hubspot", context.workspaceId, context.userEmail)));
}
