...
import { buildJiraAuthorizationUrl, getJiraConfigStatus } from "@/lib/connectors/jira";
...
export async function GET(req: NextRequest) {
  ...
  if (!getJiraConfigStatus().configured) return NextResponse.json({ error: "Jira is not configured yet. Contact support." }, { status: 503 });
  ...
  if (!context.userEmail) return NextResponse.json({ error: "A verified account email is required to connect Jira." }, { status: 400 });
  ...
  if (!workspace.data.can_use_real_connectors || workspace.data.billing_status === "preview") return NextResponse.redirect(new URL("/pricing?gate=connectors&source=jira", getAppUrl()));
  return NextResponse.redirect(buildJiraAuthorizationUrl(createProviderOAuthState("jira", context.workspaceId, context.userEmail)));
}
