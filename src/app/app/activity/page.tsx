...
export default function ActivityPage() {
  ...
  return <div className="os-page activity-page">
    ...
    <section className="p" aria-labelledby="activity-feed-title"><div className="p-head" style={{ gap: 14, alignItems: "center", flexWrap: "wrap" }}><div style={{ display: "grid", gap: 4, minWidth: 0 }}><h3 id="activity-feed-title">Workforce history</h3><span>{data ? data.summary.total === 0 ? "No events recorded in this window" : `${data.summary.total} event${data.summary.total === 1 ? "" : "s"} recorded in this window` : "Loading activity"}</span></div><div style={{ display: "flex", gap: 5, flexWrap: "wrap", justifyContent: "flex-end" }}>{filters.map((item) => <button type="button" key={item.key} className={`appr-btn ${filter === item.key ? "approve" : "edit"}`} onClick={() => { setFilter(item.key); setVisible(20); }} style={{ padding: "5px 9px", fontSize: 10.5 }}>{item.label}</button>)}</div></div>
