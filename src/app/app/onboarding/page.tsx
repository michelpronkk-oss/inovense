...
const blank: OnboardingDraft = { fullName: "", workspaceName: "", websiteUrl: "", industry: "", teamSize: "", priority: "", systems: [], noSupportedConnector: false, trialDeclined: false, step: 1 };
...
export default function OnboardingPage() {
  ...
  // Real, server-derived entitlement truth - not a draft flag. A workspace
  // stays Preview (no real OAuth, no activation, no monitoring) until an
  // explicit "Start 3-day trial" succeeds or it is on a paid plan.
  const entitlements = getEntitlements(state.workspace);
  const previewMode = !entitlements.canUseRealConnectors;
  // Truthful, not just declared: limited whenever the workspace genuinely
  // cannot activate anything right now - either it is still Preview, or the
  // user has real access but said none of the supported systems apply and
  // none actually got connected since (e.g. they changed their mind).
  const limitedMode = previewMode || (draft.noSupportedConnector && !hasCoreConnected);
  ...
  useEffect(() => { let active = true; getOnboardingDraftAction().then((result) => { if (!active) return; if (result.ok) { setDraft(result.draft); setEarlyAccessAccepted(result.earlyAccessAccepted); } else setError(result.error); setReady(true); }); return () => { active = false; }; }, []);
  ...
  // reused from onboarding exactly as /connectors uses it. On success it
  // rehydrates real workspace entitlement and, if a specific connector was
  // requested, continues straight into that connector's real setup instead
  // of leaving the user to click it again now that it is unlocked.
  async function startTrialAndContinue(pendingKey?: ConnectorKey) {
    setStartingTrial(true); setError("");
    try {
      ...
      if (pendingKey) { await openConnector(pendingKey); return; }
      setNotice("Your 3-day trial is active. Real connections are unlocked.");
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : "The trial could not be started. Please try again.");
    } finally {
      setStartingTrial(false);
    }
  }
  // Explicit "Continue in Preview": declines the trial for now so step 3 can
  // be passed without connecting anything. This never starts a trial and
  // never pretends a connector is connected.
  async function declineTrial() { setBusy(true); setError(""); const result = await saveOnboardingDraftAction({ ...draft, trialDeclined: true, step: 3 }); setBusy(false); if (!result.ok) return setError(result.error); update({ trialDeclined: true }); }
  // A recommended-system card click during Preview must never start OAuth
  // directly - it starts the trial first (remembering which connector was
  // requested) and only opens real OAuth once the trial is genuinely active.
  async function selectConnector(key: ConnectorKey) { if (previewMode) { await startTrialAndContinue(key); return; } await openConnector(key); }
  ...
  return <div className="onboarding-shell">
    ...
    <main className={`onboarding-main step-${draft.step}`}><section className="onboarding-content"><p className="onboarding-kicker"><i /> {labels[draft.step - 1]}</p>
      ...
      {draft.step === 3 && choice && <><h1>Connect the system that gives {choice.operator.replace(" Operator", "")} useful evidence.</h1><p className="onboarding-lead">Auterim works best with connected systems, but you can continue if your current tools are not supported yet.</p>{previewMode && <div className="trial-gate"><strong>Start your 3-day trial</strong><p>Connect real systems and activate your workforce when you&apos;re ready.</p><div className="trial-gate-actions"><button type="button" className="btn btn-ghost" onClick={() => void declineTrial()} disabled={busy || startingTrial}>Continue in Preview</button><button type="button" className="btn btn-primary" onClick={() => void startTrialAndContinue()} disabled={busy || startingTrial}>{startingTrial ? "Starting…" : "Start 3-day trial"}</button></div></div>}<div className="connector-section"><span>RECOMMENDED SYSTEMS</span><div className="connector-grid">{choice.core.map((key) => { const item = connectorCatalog[key]; const isConnected = connected(byConnector.get(key)); return <button type="button" key={key} className={isConnected ? "selected connected" : ""} onClick={() => !isConnected && void selectConnector(key)} disabled={busy || startingTrial || isConnected}><ProviderLogo connectorKey={key} name={item.name} fallbackLetter={item.mark} fallbackColor={item.color} box={31} size={20} radius={7} /><span><strong>{item.name}</strong><small>{isConnected ? "Connected and available to Auterim" : item.purpose}</small></span><i aria-hidden>{isConnected ? "✓" : "→"}</i></button>; })}</div></div><div className="connector-section optional"><span>OPTIONAL CONTEXT</span><div className="connector-grid">{choice.optional.map((key) => { const item = connectorCatalog[key]; const isConnected = connected(byConnector.get(key)); return <button type="button" key={key} className={isConnected ? "selected connected" : ""} onClick={() => !isConnected && void selectConnector(key)} disabled={busy || startingTrial || isConnected}><ProviderLogo connectorKey={key} name={item.name} fallbackLetter={item.mark} fallbackColor={item.color} box={31} size={20} radius={7} /><span><strong>{item.name}</strong><small>{isConnected ? "Connected" : item.purpose}</small></span><i aria-hidden>{isConnected ? "✓" : "→"}</i></button>; })}</div></div><div className="connector-section alt"><button type="button" className={`connector-alt${draft.noSupportedConnector ? " selected" : ""}`} onClick={() => void chooseNoSupportedConnector()} disabled={busy}><span>{draft.noSupportedConnector ? "✓" : "→"}</span>I use another system</button>{draft.noSupportedConnector && <p className="onboarding-note">You can continue with limited context. Auterim will show what becomes available when you connect a supported system later.</p>}</div><p className="onboarding-note">Auterim only receives the access you approve. External and higher-risk actions remain controlled.</p></>}
      ...
      {draft.step === 6 && choice && <><h1>{limitedMode ? "Your workspace is ready." : "Your workforce is ready to start watching."}</h1><p className="onboarding-lead">{limitedMode ? (previewMode ? "You're in Preview, so nothing will be marked active. Start your 3-day trial anytime to connect real systems and activate your workforce." : "No supported system is connected yet, so nothing will be marked active. Connect a system anytime from the dashboard to start real monitoring.") : canStart ? `${choice.operator} will monitor ${choice.title.toLowerCase()} through your connected systems. If no real signal exists yet, the dashboard will show exactly what it is watching.` : priorityReadiness?.nextSetupStep ?? "Finish the required connection before monitoring can start."}</p><div className="launch-summary"><div><span>CONNECTED CAPABILITY</span><b>{choice.core.filter((key) => connected(byConnector.get(key))).map((key) => connectorCatalog[key].name).join(" · ") || (limitedMode ? "Not connected yet" : "Not connected")}</b></div><div><span>OPERATOR</span><b>{limitedMode ? `${choice.operator} · Available to unlock` : choice.operator}</b></div><div><span>CONTROL</span><b>{policyMode === "approval_first" ? "Review most actions" : "Low-risk internal work"}</b></div></div><p className="control-boundary"><i /> {limitedMode ? (previewMode ? "Nothing is activated until your trial starts and a system is connected." : "Nothing is activated until a supported system is connected.") : "Nothing is fabricated. Auterim will show a real signal when one is found."}</p></>}
