    ...
    tiers: [
      {
        ...
        moveUpWhen: "Move to Extended when page count, content depth, or integration needs expand.",
      },
      {
        tier: "Extended",
        range: "$6k-$9k",
        ...
        moveUpWhen: "Move to Custom/Premium when dependency, risk, or timeline pressure becomes high.",
      },
      {
        tier: "Custom / Premium",
        range: "$10k-$18k+",
        ...
        moveUpWhen: "Do not use as default for routine builds.",
      },
    ],
    retainerPath:
    ...
    tiers: [
      {
        ...
        moveUpWhen: "Move to Extended when number of flows, integrations, or teams increases.",
      },
      {
        tier: "Extended",
        range: "$8k-$12k",
        ...
        moveUpWhen: "Move to Custom/Premium for complex orchestration, integrations, or high criticality.",
      },
      {
        tier: "Custom / Premium",
        range: "$15k-$25k+",
        ...
        moveUpWhen: "Do not position as default for standard systems engagements.",
      },
    ],
    retainerPath:
    ...
    tiers: [
      {
        ...
        moveUpWhen: "Move to Operator when channels, experiments, or content velocity grow.",
      },
      {
        ...
        moveUpWhen: "Move to Scale when execution depth and volume become materially higher.",
      },
      {
        tier: "Scale Retainer",
        range: "$5k-$8k+/mo",
        ...
        moveUpWhen: "Do not force for low-complexity or low-volume clients.",
      },
    ],
    retainerPath:
...
const TONE_STYLES: Record<SectionBlock["tone"], { label: string; cls: string }> = {
  ...
  avoid: { label: "Avoid", cls: "border-red-500/20 bg-red-500/5 text-red-300" },
  reference: { label: "Reference", cls: "border-zinc-700/60 bg-zinc-900/30 text-zinc-300" },
  guardrail: { label: "Guardrail", cls: "border-brand/30 bg-brand/10 text-brand" },
};
...
const DOC_GROUPS: DocGroup[] = [
  {
    id: "flow",
    label: "Flow",
    tagCls: "border-sky-500/30 bg-sky-500/10 text-sky-300",
    description: "Lead lifecycle, status progression, outbound prospects, onboarding boundaries, and project status.",
    keyItems: ["Lifecycle Reference", "Lead Operating Flow", "Outbound Prospects", "Onboarding"],
  },
  {
    id: "commercial",
    label: "Commercial",
    tagCls: "border-amber-500/30 bg-amber-500/10 text-amber-300",
    description: "Pricing framework, proposal workflow, action event map, payment events, and revenue visibility.",
    keyItems: ["Action Event Map", "Pricing Framework", "Payment Events", "Scenario Playbooks"],
  },
  {
    id: "delivery",
    label: "Delivery",
    tagCls: "border-emerald-500/30 bg-emerald-500/10 text-emerald-300",
    description: "Project status semantics, client portal, onboarding flow, structured content, and attention layers.",
    keyItems: ["Client Portal Light", "Lifecycle Attention", "Project Status Semantics"],
  },
  {
    id: "ai",
    label: "AI",
    tagCls: "border-violet-500/30 bg-violet-500/10 text-violet-300",
    description: "Proposal intelligence: research agent, angle agent, writer agent, and email draft flow.",
    keyItems: ["Lead Research", "Proposal Angle", "Proposal Writer"],
  },
  {
    id: "ops",
    label: "Ops",
    tagCls: "border-zinc-700/60 bg-zinc-800/40 text-zinc-300",
    description: "System layers, canonical definitions, control model, performance semantics, host routing, and build order.",
    keyItems: ["Core System Layers", "Canonical Definitions", "Performance Layer", "Operator QA"],
  },
];
...
function SectionTag({ label }: { label: HandbookSection["tag"] }) {
  return (
    <span className={`inline-flex items-center rounded-full border px-2 py-0.5 text-[10px] font-medium uppercase tracking-[0.12em] ${TAG_STYLES[label]}`}>
      {label}
    </span>
  );
}
...
function AutomationTag({ label }: { label: HandbookSection["automation"] }) {
  return (
    <span className={`inline-flex items-center rounded-full border px-2 py-0.5 text-[10px] font-medium uppercase tracking-[0.12em] ${AUTOMATION_STYLES[label]}`}>
      {label}
    </span>
  );
}
  ...
  return (
    <div className={`rounded-xl border p-3 ${tone.cls}`}>
      <div className="mb-2 flex items-center justify-between gap-2">
        <p className="text-[10px] font-medium uppercase tracking-[0.13em] opacity-80">{tone.label}</p>
        <span className="text-[10px] uppercase tracking-[0.12em] opacity-70">{block.title}</span>
      </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-zinc-800/80 bg-zinc-900/30">
        <details>
          <summary className="cursor-pointer list-none border-b border-zinc-800/70 bg-zinc-900/70 px-4 py-3 marker:hidden [&::-webkit-details-marker]:hidden">
            ...
            <p className="mt-1 max-w-3xl text-xs leading-relaxed text-zinc-500">{section.summary}</p>
          </summary>
...
function SectionHeader({
  ...
  tag: HandbookSection["tag"];
  automation: HandbookSection["automation"];
  title: string;
  summary: string;
}) {
  return (
    <div className="border-b border-zinc-800/70 bg-zinc-900/70 px-4 py-3">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <AutomationTag label={automation} />
...
function FlowGroupSections() {
  const flowSections = SECTIONS.filter((s) =>
    ["lead-operating-flow", "outbound-prospects", "onboarding-and-activation"].includes(s.id)
  );
  return (
    <div className="space-y-4">
      {/* Lifecycle Reference */}
      <section id="flow-reference" className="scroll-mt-6">
        <div className="overflow-hidden rounded-2xl border border-zinc-800/80 bg-zinc-900/30">
          <SectionHeader tag="Flow" automation="Mixed" title="Lifecycle Reference" summary="Use this flow as the operational spine. Status should mirror actual progression." />
          <div className="space-y-2 p-4">
            {LEAD_FLOW_STEPS.map((item, index) => (
              <div key={item.step} className="grid gap-2 rounded-xl border border-zinc-800/70 bg-zinc-950/35 p-3 md:grid-cols-[28px_1fr_auto]">
      ...
      {flowSections.map((s) => <HandbookSectionCard key={s.id} section={s} />)}

      {/* Project Status Reference */}
      <section id="project-status-reference" className="scroll-mt-6">
        <div className="overflow-hidden rounded-2xl border border-zinc-800/80 bg-zinc-900/30">
          <SectionHeader tag="Delivery" automation="Manual" title="Project Status Reference" summary="Project status is delivery tracking only. Keep it separate from lifecycle and payment states." />
          <div className="grid gap-3 p-4 md:grid-cols-2 lg:grid-cols-5">
            {PROJECT_STATUS_REFERENCE.map((item) => (
              <div key={item.value} className="rounded-xl border border-zinc-800/70 bg-zinc-950/35 p-3">
...
function CommercialGroupSections() {
  const commercialSections = SECTIONS.filter((s) =>
    ["proposal-and-commercial", "payment-and-confirmation", "currency-and-market-handling", "revenue-visibility"].includes(s.id)
  );
  return (
    <div className="space-y-4">
      {/* Action Event Map */}
      <section id="action-event-map" className="scroll-mt-6">
        <div className="overflow-hidden rounded-2xl border border-zinc-800/80 bg-zinc-900/30">
          <SectionHeader tag="Ops" automation="Mixed" title="Action Event Map" summary="Verify what each critical action writes, which email event fires, and what the next operator decision should be." />
          <div className="space-y-2 p-4">
            {ACTION_EVENT_MATRIX.map((row) => (
              <article key={row.action} className="grid gap-2 rounded-xl border border-zinc-800/70 bg-zinc-950/35 p-3 md:grid-cols-[1.4fr_1.2fr_1.4fr_1.4fr]">
                <div>
      ...
      {/* Pricing Framework */}
      <section id="pricing-framework" className="scroll-mt-6">
        <div className="overflow-hidden rounded-2xl border border-zinc-800/80 bg-zinc-900/30">
          <SectionHeader tag="Commercial" automation="Manual" title="Pricing Framework" summary="English-first internal USD anchors with framework tiers and clean upgrade logic. Convert to local client currency for client-facing communication where needed." />
          <div className="space-y-3 p-4">
            {LANE_PRICING_ANCHORS.map((lane) => (
              <article key={lane.lane} className="rounded-xl border border-zinc-800/70 bg-zinc-950/35 p-3.5">
                <div className="grid gap-2 md:grid-cols-[1.4fr_1fr]">
                  <div>
                    <p className="text-[10px] uppercase tracking-[0.12em] text-zinc-600">Lane</p>
                    <p className="mt-1 text-sm font-medium text-zinc-200">{lane.lane}</p>
                    <p className="mt-1 text-xs leading-relaxed text-zinc-500">{lane.pricingModel}</p>
                  </div>
                  <div>
                    <p className="text-[10px] uppercase tracking-[0.12em] text-zinc-600">Quote First</p>
                    <p className="mt-1 text-xs leading-relaxed text-zinc-300">{lane.quoteFirst}</p>
                  </div>
                </div>
                <div className="mt-3 grid gap-2 md:grid-cols-3">
                  {lane.tiers.map((tier) => (
                    <div key={`${lane.lane}-${tier.tier}`} className="rounded-lg border border-zinc-800/70 bg-zinc-900/35 p-3">
                      <p className="text-[10px] uppercase tracking-[0.12em] text-zinc-600">{tier.tier}</p>
                      <p className="mt-1 text-sm font-semibold text-zinc-100">{tier.range}</p>
                      <p className="mt-1 text-xs leading-relaxed text-zinc-400">{tier.scope}</p>
                      <p className="mt-1 text-xs leading-relaxed text-zinc-500"><span className="text-zinc-400">Use when: </span>{tier.useWhen}</p>
                      <p className="mt-1 text-xs leading-relaxed text-zinc-500"><span className="text-zinc-400">Move up when: </span>{tier.moveUpWhen}</p>
                    </div>
                  ))}
                </div>
                <p className="mt-3 text-xs leading-relaxed text-zinc-400"><span className="text-zinc-300">Retainer path: </span>{lane.retainerPath}</p>
              </article>
            ))}
          </div>
          <div className="grid gap-3 border-t border-zinc-800/70 p-4 md:grid-cols-2 xl:grid-cols-4">
            <BlockCard block={{ title: "Quote flow", tone: "do", items: QUOTE_FLOW_RULES }} />
            <BlockCard block={{ title: "Anti-chaos quoting rules", tone: "guardrail", items: PRICING_GUARDRAILS }} />
            <BlockCard block={{ title: "Retainer logic", tone: "do", items: RETAINER_DECISION_RULES }} />
            <BlockCard block={{ title: "What not to do", tone: "avoid", items: PRICING_ANTI_PATTERNS }} />
          </div>
        </div>
      </section>

      {commercialSections.map((s) => <HandbookSectionCard key={s.id} section={s} />)}

      {/* Scenario Playbooks */}
      <section id="scenario-playbooks" className="scroll-mt-6">
        <div className="overflow-hidden rounded-2xl border border-zinc-800/80 bg-zinc-900/30">
          <SectionHeader tag="Ops" automation="Manual" title="Scenario Playbooks" summary="Fast guidance for frequent edge cases so operators can move without guessing." />
          <div className="grid gap-3 p-4 md:grid-cols-2">
            {SCENARIO_PLAYBOOKS.map((scenario) => (
              <article key={scenario.title} className="rounded-xl border border-zinc-800/70 bg-zinc-950/35 p-3.5">
...
function DeliveryGroupSections() {
  const deliverySections = SECTIONS.filter((s) =>
    ["project-status-semantics", "onboarding-and-activation", "client-portal-light", "structured-client-content-rendering", "lifecycle-attention-layer", "weekly-operating-summary"].includes(s.id)
  );
  return (
    <div className="space-y-4">
      {deliverySections.map((s) => <HandbookSectionCard key={s.id} section={s} />)}
    </div>
  );
}
...
function AIGroupSections() {
  ...
  return (
    <div className="space-y-4">
      {aiSections.map((s) => <HandbookSectionCard key={s.id} section={s} />)}
    </div>
  );
}
...
function OpsGroupSections() {
  const opsSections = SECTIONS.filter((s) =>
    ["locale-and-language", "outbound-prospects", "performance-decision-layer", "host-surface-routing", "traffic-and-performance-definitions", "deliberate-non-features", "operator-qa", "lifecycle-attention-layer", "weekly-operating-summary"].includes(s.id)
  );
  return (
    <div className="space-y-4">
      {/* Operating State Now */}
      <section id="operating-state-now" className="scroll-mt-6">
        <div className="overflow-hidden rounded-2xl border border-zinc-800/80 bg-zinc-900/30">
          <SectionHeader tag="Ops" automation="Mixed" title="Operating State Now" summary="Current live state beyond the earlier launch baseline. Treat this as real system truth." />
          <div className="grid gap-3 p-4 md:grid-cols-2">
            {OPERATING_STATE_NOW.map((item) => (
              <article key={item.title} className="rounded-xl border border-zinc-800/70 bg-zinc-950/35 p-3.5">
      ...
      {/* Core System Layers */}
      <section id="core-system-layers" className="scroll-mt-6">
        <div className="overflow-hidden rounded-2xl border border-zinc-800/80 bg-zinc-900/30">
          <SectionHeader tag="Ops" automation="Mixed" title="Core System Layers" summary="Layer map for operators across public, admin, proposal, portal, prospects, performance, and docs." />
          <div className="space-y-2 p-4">
            {CORE_SYSTEM_LAYERS.map((row) => (
              <article key={row.layer} className="grid gap-2 rounded-xl border border-zinc-800/70 bg-zinc-950/35 p-3 md:grid-cols-[1fr_1fr_1.2fr_1.2fr]">
                <div>
      ...
      <section id="canonical-definitions" className="scroll-mt-6">
        <div className="overflow-hidden rounded-2xl border border-zinc-800/80 bg-zinc-900/30">
          <SectionHeader tag="Ops" automation="Manual" title="Canonical Definitions" summary="Non-negotiable terminology for lifecycle, performance, prospects, and client-facing naming." />
          <div className="space-y-2 p-4">
            {CANONICAL_DEFINITIONS.map((row) => (
              <article key={row.term} className="grid gap-2 rounded-xl border border-zinc-800/70 bg-zinc-950/35 p-3 md:grid-cols-[0.8fr_1.4fr_1.4fr]">
                <div>
      ...
      {/* Control Model */}
      <section id="control-model" className="scroll-mt-6">
        <div className="overflow-hidden rounded-2xl border border-zinc-800/80 bg-zinc-900/30">
          <SectionHeader tag="Ops" automation="Mixed" title="Control Model" summary="Keep operator ownership and automation boundaries explicit at every stage." />
          <div className="grid gap-3 p-4 md:grid-cols-3">
            {CONTROL_MODEL.map((card) => (
              <article key={card.title} className="rounded-xl border border-zinc-800/70 bg-zinc-950/35 p-3.5">
      ...
      {/* Manual vs Automatic */}
      <section id="manual-vs-automatic" className="scroll-mt-6">
        <div className="overflow-hidden rounded-2xl border border-zinc-800/80 bg-zinc-900/30">
          <SectionHeader tag="Ops" automation="Mixed" title="Manual vs Automatic" summary="Quick reference for what operators own directly and what the CRM updates automatically." />
          <div className="space-y-2 p-4">
            {MANUAL_AUTOMATION_REFERENCE.map((item) => (
              <article key={item.area} className="grid gap-2 rounded-xl border border-zinc-800/70 bg-zinc-950/35 p-3 md:grid-cols-[1fr_1.4fr_1.4fr]">
                <div>
      ...
      {/* Current Build Order */}
      <section id="current-build-order" className="scroll-mt-6">
        <div className="overflow-hidden rounded-2xl border border-zinc-800/80 bg-zinc-900/30">
          <SectionHeader tag="Ops" automation="Manual" title="Current Build Order" summary="Ordered by current operational value, not vanity roadmap volume." />
          <div className="space-y-2 p-4">
            {CURRENT_BUILD_ORDER.map((row, index) => {
              ...
              return (
                <article key={row.step} className="grid gap-2 rounded-xl border border-zinc-800/70 bg-zinc-950/35 p-3 md:grid-cols-[28px_1.2fr_1.2fr_1.2fr]">
                  <span className="text-sm font-semibold tabular-nums text-zinc-500">{index + 1}</span>
                  <div>
                    ...
                    <span className={`mt-2 inline-flex items-center rounded-full border px-2 py-0.5 text-[10px] font-medium uppercase tracking-[0.12em] ${statusClass[row.status]}`}>
                      {row.status}
                    </span>
                  </div>
                  <div>
      ...
      {opsSections.map((s) => <HandbookSectionCard key={s.id} section={s} />)}
    </div>
  );
}
...
function GroupSections({ groupId }: { groupId: DocGroupId }) {
  switch (groupId) {
    case "flow": return <FlowGroupSections />;
    case "commercial": return <CommercialGroupSections />;
    case "delivery": return <DeliveryGroupSections />;
    case "ai": return <AIGroupSections />;
    case "ops": return <OpsGroupSections />;
  }
}
    ...
    <div className="space-y-6">
      <div>
        <div className="mb-2 flex items-center gap-2">
          ...
          <span className="text-[10px] text-zinc-600">Operating Handbook</span>
        </div>
        <h1 className="text-lg font-semibold tracking-tight text-zinc-100">CRM Command Reference</h1>
        <p className="mt-1.5 max-w-xl text-xs leading-relaxed text-zinc-500">
          Source of truth for lifecycle execution, commercial workflow, delivery, AI stack, and ops semantics.
...
function GroupDetailView({
  ...
  return (
    <div className="space-y-5">
      ...
      <div>
        <h1 className="text-lg font-semibold text-zinc-100">{group.label}</h1>
...
export function DocsClient() {
  ...
  const group = activeGroup ? DOC_GROUPS.find((g) => g.id === activeGroup) ?? null : null;

  return (
    <div className="mx-auto max-w-4xl">
      {group ? (
        <GroupDetailView group={group} onBack={() => setActiveGroup(null)} />
      ) : (
        <DocsHome onOpen={setActiveGroup} />
      )}
    </div>
  );
}
