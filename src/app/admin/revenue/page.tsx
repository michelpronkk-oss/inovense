...
export default async function RevenuePage() {
  ...
  return <div className="admin-command-center admin-detail-page">
    ...
    <section className="admin-kpi-grid admin-kpi-grid-four"><Metric label="MRR" value={currency(data.mrr)} note={data.mrrNote} /><Metric label="Active subscriptions" value={number(data.subscriptionStates.active)} note="Workspace billing state" /><Metric label="Trialing" value={number(data.trials.active)} note={data.trials.available ? "Immutable first-time trial records" : "Trial migration not applied"} /><Metric label="Trial conversion" value={data.trials.conversionRate === null ? "Unavailable" : `${(data.trials.conversionRate * 100).toFixed(1)}%`} note={data.trials.available ? "Converted / started trials" : "Trial migration not applied"} /></section>
