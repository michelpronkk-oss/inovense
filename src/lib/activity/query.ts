...
export async function getWorkforceActivity(input: { workspaceId: string; range: Range; limit?: number }): Promise<WorkforceActivityPage> {
  ...
  if (approvals.error || runs.error || logs.error) throw new Error("Workforce activity is temporarily unavailable.");
  return normalizeWorkforceActivity({ approvals: approvals.data ?? [], runs: runs.data ?? [], logs: logs.data ?? [], rangeStart: start, limit: input.limit });
}
