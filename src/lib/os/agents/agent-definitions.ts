...
export const OS_AGENT_DEFINITIONS: OSAgentDefinition[] = [
  ...
  },
  {
    ...
    description: "Monitors support requests, prepares bounded replies and escalations, and observes resolution evidence.",
    systemInstructions: "Draft first, send only when approved by policy.",
    goals: ["Faster support response preparation"],
    requiredConnectors: [],
    allowedTools: ["gmail.createDraft", "microsoft.createDraft", "zendesk.readTickets", "zendesk.prepareReply", "intercom.readConversations", "intercom.prepareReply", "memory.search", "logs.write"],
