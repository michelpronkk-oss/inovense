...
const mobileGrowthChannels = [
  { label: "SEO", fill: 84, trend: "+12%" },
  { label: "Betaald", fill: 68, trend: "+7%" },
  { label: "Content", fill: 91, trend: "+19%" },
];

function MobileNlGrowthVisual() {
  return (
    <div className="relative mx-auto w-full max-w-sm select-none lg:hidden">
      ...
      <div className="relative overflow-hidden rounded-2xl border border-zinc-800/80 bg-zinc-950/70 p-4 shadow-[0_18px_70px_rgba(0,0,0,0.42)]">
        <div className="mb-4 flex items-center justify-between border-b border-zinc-800/70 pb-3">
          <span className="text-xs font-medium text-brand">Growth overzicht</span>
          <span className="text-[11px] font-semibold text-brand">+24%</span>
        </div>
        <div className="space-y-2.5">
          {mobileGrowthChannels.map(({ label, fill, trend }) => (
            <div key={label} className="flex items-center gap-3">
              ...
              <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-zinc-800/80">
                <div className="h-full rounded-full bg-brand/55" style={{ width: `${fill}%` }} />
              </div>
              <span className="w-8 shrink-0 text-right text-[10px] font-medium text-brand">{trend}</span>
            </div>
          ))}
        </div>
        <div className="mt-4 rounded-xl border border-zinc-800 bg-zinc-900/55 p-3 text-left">
          <p className="text-xs font-medium text-zinc-300">Gekwalificeerde leads</p>
          <p className="mt-1 text-[11px] leading-relaxed text-zinc-600">
            Niet willekeurige klikken en impressies.
          </p>
        </div>
      </div>
    </div>
