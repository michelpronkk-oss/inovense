...
> = {
  build: {
    label: "Build",
    href: "/nl/build",
    description: "Websites en digitale producten",
  },
  systems: {
    label: "Systems",
    href: "/nl/systems",
    description: "AI-workflows en automatisering",
  },
  growth: {
    label: "Growth",
    href: "/nl/growth",
    description: "SEO, content en betaalde groei",
  },
};
...
export default function NlRelatedServices({ current }: { current: ServiceKey }) {
      ...
      <div className="mx-auto max-w-6xl px-6">
        <div className="flex flex-col gap-6 sm:flex-row sm:items-start">
          ...
          <div className="flex flex-wrap gap-x-8 gap-y-5">
            ...
            <Link href="/nl/process" className="group flex flex-col gap-0.5">
              <span className="flex items-center gap-1.5 text-sm font-medium text-zinc-300 transition-colors group-hover:text-zinc-50">
                Proces
                <ArrowRight />
              </span>
              <span className="text-xs text-zinc-600 transition-colors group-hover:text-zinc-500">
                Hoe elk project werkt
              </span>
            </Link>
          </div>
        </div>
