...
export default function V3Page() {
  ...
  return <div className="auterim-v3-page homepage-recomposed">
    ...
    <section className="sec homepage-connectors" id="connectors"><div className="wrap">
      <Head label="Your existing stack" title="Keep the tools you trust. Add Auterim on top." body="Connect the tools you already use. Auterim combines their context and turns important signals into governed work." />
    ...
    <section className="close homepage-final-cta"><div className="wrap close-in rv"><div className="close-main"><span className="lbl">Start with one Operator</span><h2>Find your first Operator.</h2><p>Choose what Auterim should handle first. Your Operator prepares the work and holds consequential actions for approval.</p><div className="close-actions"><button type="button" onClick={(event) => openEarlyAccess({ trigger: event.currentTarget })} className="btn btn-a">{hasSubmitted ? "Request received" : <>Request early access <span className="arrow">→</span></>}</button><a href="#how" className="close-run">See how it works <span>→</span></a></div><p className="close-note">Early Access is open now. We review each use case before enabling a workspace.</p></div><FinalCtaVisual /></div></section>
    <V3Footer />
  </div>;
}
